...
- **A container's policy is apart from its data** (review 2026-09-25, P1):
  ...
  storage is written through every zone's home layer, and a program in a zone
  could bind a container to the host's network (`network = unconfined`) or
  grant it a directory, and the next launch from the menu obeyed; the config
  dir is read-only in zones. The files move once, on the host, at the first
