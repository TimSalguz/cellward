...
/// `vpn-zone nix-daemon|host-files|camera|audio-manager <zone>
/// <on>|<off>|default`.
...
/// `cellward screencast <zone> yes|no|ask|default` (`crate::screencast`): the
...
fn zone_screencast(tools: &Tools, args: &[OsString]) -> u8 {
    ...
    const USAGE: &str = "cellward screencast <зона> yes|no|ask|default";
    let (Some(name), Some(value)) = (args.first(), args.get(1)) else {
        eprintln!("{USAGE}");
        return 1;
    };
            ...
            let (Some(selector), Some(key), Some(value)) =
                ...
                eprintln!(
                    "cellward container set <контейнер> network <сеть|ask> | x11 on|off | \
                     home private|layer|main | color <#rrggbb>|default | \
                     microphone yes|no|ask|default"
                );
                return 1;
            };
            if key == "microphone" {
                let setting = match value.as_str() {
                    ...
                    word => match crate::microphone::Setting::parse(word) {
                        ...
                        None => {
                            eprintln!("microphone: yes, no, ask или default");
                            return 1;
                        }
                    },
                };
                return match crate::container::set_microphone(tools, selector, setting) {
                    Ok(()) => {
                        match setting {
                            Some(s) => println!(
                                "микрофон контейнера {selector}: {} (действует сразу; Nix зоны \
                                 важнее)",
                                s.as_str()
                            ),
                            None => {
                                println!("у контейнера {selector} снова микрофон как у его зоны")
                            }
                        }
                        0
                    }
...
/// `vpn-zone frame color <zone> <#rrggbb>|default`: the frame of the zone's
