...
/// `vpn-zone nix-daemon|host-files|camera <zone> <on>|<off>|default`.
...
/// `vpn-zone microphone <zone> yes|no|ask|default` (`crate::microphone`): the
...
fn zone_microphone(tools: &Tools, args: &[OsString]) -> u8 {
    ...
    // Said for what it is: the switch of the sound filter, the PulseAudio
    // path. What goes around it is named beside it (review 2026-09-25) — a
    // "no" read as the zone's boundary would be a promise it does not keep.
    let said = match setting {
        Setting::Yes => "программы зоны записывают микрофон без вопроса",
        Setting::No => "через звуковой сервер (pulse) микрофон программам зоны недоступен",
        Setting::Ask => {
    ...
    if setting != Setting::Yes {
        println!(
            "  мимо этого переключателя: сырой pipewire-0 — он открыт в каждой зоне \
             (свой контекст PipeWire — в планах)"
        );
        let (hermetic, _) = crate::hermetic::zone_setting(&dir, &tools.config, &name);
        if !hermetic {
            println!(
                "  и systemd --user хоста: зона не герметична, её программа запустит запись \
                 на хосте или перепишет эту настройку сама (vpn-zone hermetic {name} on)"
            );
        }
    }
    0
...
/// `vpn-zone frame show|hide`, `vpn-zone frame width <px>|default`,
/// `vpn-zone frame color <zone> <#rrggbb>|default`: the border of the zone's
/// colour the Wayland proxy draws around its programs' windows
/// (`crate::frame`, `docs/WINDOW-FRAME.md` §0а). Each takes effect for
/// windows opened after it: the colour and the width when a program is
/// launched, the switch when it connects.
fn frame(tools: &Tools, args: &[OsString]) -> u8 {
    use crate::container::Source;
    use crate::frame::{Rgb, COLOR_FILE, MAX_WIDTH, SWITCH_SETTING, WIDTH_SETTING};
