            ...
            let (Some(selector), Some(key), Some(value)) =
                ...
                eprintln!(
                    "cellward container set <контейнер> network <сеть|ask> | x11 on|off | \
                     ...
                     microphone zone (по умолчанию)|yes|no|ask"
                );
                return 1;
            };
            if key == "microphone" {
                // `zone`: none of its own — the zone's. Not a value of the
                // three, so named for what it is (the owner, 2026-09-26: no
                // bare `default` where a value can be marked as one).
                let setting = match value.as_str() {
                    ...
                    word => match crate::microphone::Setting::parse(word) {
                        ...
                        None => {
                            eprintln!(
                                "microphone: zone (по умолчанию — как у зоны), yes, no или ask"
                            );
                            return 1;
                        }
                    },
