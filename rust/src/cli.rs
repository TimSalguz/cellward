...
/// `vpn-zone hermetic <zone> on|off`: the prototype of `docs/HERMETICITY.md`
/// §7 C — the runtime directory closed, the session bus filtered, the broker as
/// the way out. Takes effect when the zone next comes up.
fn zone_hermetic(tools: &Tools, args: &[OsString]) -> u8 {
    let (Some(name), Some(value)) = (args.first(), args.get(1)) else {
        eprintln!("vpn-zone hermetic <зона> on|off");
        return 1;
    };
    let dir = tools.state.join(name);
    if !dir.is_dir() {
    ...
    match value.to_str() {
        Some("on") => {
            ...
            println!(
                "зона {name} герметична (прототип): без systemd --user, сессионная шина через фильтр, \
        ...
        Some("off") => {
            let _ = fs::remove_file(&marker);
            println!("зона {name} не герметична{restart}");
            0
        }
        _ => {
            eprintln!("vpn-zone hermetic <зона> on|off");
