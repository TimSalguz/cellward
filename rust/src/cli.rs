...
fn zone_microphone(tools: &Tools, args: &[OsString]) -> u8 {
    ...
    let said = match setting {
        ...
        Setting::No => "микрофон программам зоны недоступен",
        Setting::Ask => {
            "при записи с микрофона программа зоны спросит: один раз, всегда или отказать"
        }
    };
    let from = match source {
