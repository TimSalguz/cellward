...
# Рабочий каталог (docs/GOTCHAS.md §10a): nsenter, входя в mount namespace,
# делает chdir("/"), и терминал в зоне открывался в корне. Две проверки, и
# вторая — главная: маркер есть ТОЛЬКО в верхнем слое, так что увидеть его
# относительным путём можно, лишь если chdir сделан ПОСЛЕ монтирования оверлея.
step "Рабочий каталог: программа в зоне стартует там же, где её запустили"
cwdout=$(cd "$WORK" && "$VPN_ZONE" run smoke -- pwd)
[ "$cwdout" = "$WORK" ] || fail "в зоне рабочий каталог «$cwdout», а запускали из «$WORK»"
cwdout=$(cd "$HOME/.config" && "$VPN_ZONE" run smoke --profile "$TEST_PROFILE" -- \
  sh -c 'test -f vpn-smoke-marker && echo В-СЛОЕ')
[ "$cwdout" = "В-СЛОЕ" ] \
