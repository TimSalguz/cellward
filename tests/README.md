...
6. **vm-audio** — `tests/vm-audio.nix`: настоящие PipeWire и WirePlumber
   пользователя в VM с политикой из модуля NixOS
   (`services.vpn-zones.pipewirePolicy`), нулевые выход и микрофон; из
   ...
   — ноль байт; микрофон — по `vpn-zone microphone`; виртуальный выход зоны
...
- пикер сети без графики: у синтетического ключа `smoke-pickapp` в `.last`
  ...
  stderr, записать метку и запустить команду через `vpn-zone run` — в реестре
